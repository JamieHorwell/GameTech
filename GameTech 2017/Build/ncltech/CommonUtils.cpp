	...
	const Vector4& color)
{
	...
	if (physics_enabled)
	{
		...
		else
		{
			...
			//and set broadphase shape
			CollisionShape* bColshape = new SphereCollisionShape(radius);
			pnode->SetBroadphaseShape(bColshape);
	...
	const Vector4& color)
{
	...
	if (physics_enabled)
	{
		...
		else
		{
			...
			pnode->SetInverseInertia(pColshape->BuildInverseInertia(inverse_mass));
			//and set broadphase shape
			CollisionShape* bColshape = new SphereCollisionShape(halfdims.Length());
			pnode->SetBroadphaseShape(bColshape);
		}
	}
