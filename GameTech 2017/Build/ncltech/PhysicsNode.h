...
public:
	...
	inline CollisionShape*		GetBroadPhaseShape()		const { return sphereBroadShape; }
	...
	inline void SetBroadphaseShape(CollisionShape* broadShape) {
		if (broadShape) {
			//delete old broadShape
			delete sphereBroadShape;
			sphereBroadShape = nullptr;
			sphereBroadShape = broadShape;
			sphereBroadShape->SetParent(this);
		}
	}
	
	inline void setCollided(bool collided) { this->collidedThisFrame = collided; };

	inline void addForce(const Vector3& f) { this->force += f; };
...
//Added in Tutorial 4/5
	...
	//used for broadphase culling
	CollisionShape*				sphereBroadShape;
